  ...
  function renderInfoPanel(container, chartData) {
    ...
    if (majorAspects.length > 0) {
      ...
      majorAspects.forEach((a, idx) => {
        const row = createAspectRow(a, idx);
    ...
    if (minorAspects.length > 0) {
      ...
      minorAspects.forEach((a, idx) => {
        const row = createAspectRow(a, idx + majorAspects.length);
        aspectList.appendChild(row);
      });
    }
    ...
    const analysisSection = createSection('Element & Modalite Dağılımı', 'analysis');
